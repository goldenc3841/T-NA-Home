...
export default function SidebarNav({ profileName, profileRole }: SidebarNavProps) {
  ...
  return (
    <aside className="w-64 bg-slate-900/60 border-r border-white/5 flex flex-col justify-between shrink-0 glass">
      {/* Top Header */}
      <div>
        <div className="h-16 flex items-center px-6 border-b border-white/5">
          <Link href="/dashboard" className="flex items-center gap-2">
            <span className="h-8 w-8 rounded-lg bg-gradient-to-tr from-violet-600 to-indigo-500 flex items-center justify-center font-bold text-white shadow-lg shadow-violet-500/20">
              T
            </span>
      ...
      <div className="p-4 border-t border-white/5 space-y-3">
        <div className="flex items-center gap-3 px-2 py-1.5 rounded-lg bg-white/5 border border-white/5">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-xs font-semibold text-slate-200 truncate" title={profileName}>
              {profileName}
            </div>
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 transition-all cursor-pointer"
        >
